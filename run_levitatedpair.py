...
from transformers import (WEIGHTS_NAME, BertConfig,
                                  ...
                                  BertForACEBothOneDropoutLeviPair_blinear,
                                  BertForACEBothOneDropoutLeviPair_blinear_1,
                                  )
...
MODEL_CLASSES = {
    ...
    'bert_blinear': (BertConfig, BertForACEBothOneDropoutLeviPair_blinear, BertTokenizer),
    'bert_blinear_1': (BertConfig, BertForACEBothOneDropoutLeviPair_blinear_1, BertTokenizer),
    'bert_blinear_typemarker': (BertConfig, BertForACEBothOneDropoutLeviPair_blinear_1, BertTokenizer),
}
...
data_dir={'data_path_loss':'train_re_levitated_loss.txt','data_name_loss':'train_re_levitated_loss','data_path_f1':'train_re_levitated_f1.txt','data_name_f1':'train_re_levitated_f1'}
...
class ACEDataset(Dataset):
    ...
    def initialize(self):
        ...
        for l_idx, line in enumerate(f):
            ...
            for n in range(len(subword_sentence_boundaries) - 1):
                ...
                for sub_start, sub_end, sub_label in entities:#遍历每一个实体
                    ...
                    for start, end, obj_label in sentence_ners:#遍历每一个实体
                ...
                for i in range(0, len(cur_ins), dL):
                    examples = cur_ins[i : i + dL]#一个example就是一个实体对关系
                    item = {
                        'index': (l_idx, n),
    ...
    def __getitem__(self, idx):
        ...
        for x_idx, item in enumerate(entry['examples']):
            ...
            #改进，若使用typemarker
            if self.use_typemarker:#如果使用了标签
                l_m_sub = '[unused%d]' % ( m1[2] )#m1指的是subject
                r_m_sub = '[unused%d]' % ( m1[2] + len(self.ner_label_list))
                l_m_ob = '[unused%d]' % ( m2[2] + len(self.ner_label_list) * 2)#m2[2]指代的是object的label信息
                r_m_ob = '[unused%d]' % ( m2[2] + len(self.ner_label_list) * 3)

                l_m_sub = self.tokenizer._convert_token_to_id(l_m_sub)#把这个标签unusedx换成了对应的vocab表中的索引
                r_m_sub= self.tokenizer._convert_token_to_id(r_m_sub)
                l_m_ob = self.tokenizer._convert_token_to_id(l_m_ob)
                r_m_ob = self.tokenizer._convert_token_to_id(r_m_ob)

                input_ids[w1] = l_m_sub#将object的类型标签的索引加入到相应的位置，而不再是unused0和unused1
                input_ids[w2] = r_m_sub
                input_ids[w3] = l_m_ob
                input_ids[w4] = r_m_ob


            position_ids[w1] = m1[0]#subject
            position_ids[w2] = m1[1]

            position_ids[w3] = m2[0]#object
            position_ids[w4] = m2[1]

            for xx in [w1, w2, w3, w4]:#注意力矩阵，每个subject和object可以相互关注
