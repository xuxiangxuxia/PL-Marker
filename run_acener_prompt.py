...
from transformers import (WEIGHTS_NAME, BertConfig,
                          ...
                          RobertaForSpanMarkerNER_prompt,
                          AlbertForSpanNER,
...
MODEL_CLASSES = {
    ...
    'robertaspanmarkerprompt': (RobertaConfig, RobertaForSpanMarkerNER_prompt, RobertaTokenizer),
        ...
        for l_idx, line in enumerate(f):
            ...
            for n in range(len(subword_sentence_boundaries) - 1):
                ...
                if 'roberta' in self.args.model_type:
                    target_tokens = [tokenizer.cls_token] + target_tokens + ["[UNK]", "[UNK]", "is", "a", "<mask>","entity", "."] + [tokenizer.sep_token]
                else:
                ...
                #注，此处要修改，将后面加进来的prompt不考虑进实体对，因此在考虑实体对的边界时，全部都减7
...
def train(args, model, tokenizer):
    ...
    train_dataset = ACEDatasetNER(tokenizer=tokenizer,
                                  args=args)  # 其中包含每一个句子的，任意两个位置之间的实体类别信息,注:sentence中包含了几个句子，并不是一个，而每个example中的这种实体类型关系长度有限，因此可能会分割成很多块
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        indexs = batch[-2]  # 一个batch_size的样本在文件中的索引
        batch_m2s = batch[-1]  ## (1, 1)subword边界16*1
        ...
        with torch.no_grad():
            ...
            for i in range(len(indexs)):  # 共有多少个样本即batchsize
