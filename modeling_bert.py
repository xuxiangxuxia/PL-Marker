...
class BertForACEBothOneDropoutSpanSub(BertPreTrainedModel):#这个是考虑了span的模型
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
    ...
    def forward(
        ...
        m2_span_start_states = hidden_states[torch.arange(bsz).unsqueeze(-1), mention_pos[:, :, 0]]#mention_pos记录的是subject和object之间的相对位置关系，即是否有重叠
        m2_span_end_states = hidden_states[torch.arange(bsz).unsqueeze(-1), mention_pos[:, :, 1]]#0，1代表的是left和right，通过与原始的left即object的subword的边界值对比可以得到信息，2代表的是object的种类
...
class BertForACEBothOneDropoutSub_1(BertPreTrainedModel):#re修改
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
        ...
        #改进之处，使用双线性变换层，综合考虑subject和objec之间的关系
    ...
    def forward(
        ...
        hidden_states = outputs[0]  # bert的输出，0是最后一层encoder的输出   1是cls符号的最后一层输出
        hidden_states = self.dropout(hidden_states)  #8*288*768
        seq_len = self.max_seq_length
        bsz, tot_seq_len = input_ids.shape
        ent_len = (tot_seq_len - seq_len) // 2  #16

        #改进之处，因为后续的数值暂时只能由列表存储
        re_prediction_scores=[]


        e1_hidden_states = hidden_states[:, seq_len:seq_len + ent_len]
        e2_hidden_states = hidden_states[:, seq_len + ent_len:]  #8*16*768  object的悬浮标记

        feature_vector = torch.cat([e1_hidden_states, e2_hidden_states], dim=2)  #这个只是object的特征

        ner_prediction_scores = self.ner_classifier(feature_vector)  # 预测object的ner的类型

        m1_start_states = hidden_states[torch.arange(bsz), sub_positions[:, 0]]
        m1_end_states = hidden_states[torch.arange(bsz), sub_positions[:, 1]]#
        m1_states = torch.cat([m1_start_states, m1_end_states], dim=-1)#得到的是subject实体的的开始结束信息



        m1_scores = self.re_classifier_m1(m1_states)  # bsz, num_label 8*13
        m2_scores = self.re_classifier_m2(feature_vector)  # bsz, ent_len, num_label 8*16*13,将8*16*1536转化成这样
        #re_prediction_scores = m1_scores.unsqueeze(1) + m2_scores  # 升维后再相加 8*16*13


        #改进方法,先降维是为了减少运算量，GELU与随机正则化有关，使用该函数可以让得到的向量变得更加规范
        m1 = F.gelu(self.reduce_dim(m1_states))
        m2 = F.gelu(self.reduce_dim(feature_vector))
        dimenson = m1_states.shape[0]
        for i in range(0,dimenson):
            m2_1 = m2[i]
            m1_1 = m1[i]
            for num in range(0, 16):
                m2_2 = m2_1[num]
                re_prediction_scores.append(self.blinear(m1_1, m2_2))

        re_prediction_scores = torch.stack(re_prediction_scores)#转化为tensor
        re_prediction_scores = torch.mul(re_prediction_scores, 1 / 16)
        re_prediction_scores = torch.reshape(re_prediction_scores,(dimenson,16,13))#将原来的tensor改变形状




        outputs = (re_prediction_scores, ner_prediction_scores) + outputs[2:]  # Add hidden states and attention if they are here
        # re阶段的预测值并不像想象的一样，在这个方法中是分别计算suject和object的re预测得分，合起来作为最终的预测得分
        if labels is not None:  # 计算损失
            loss_fct_re = CrossEntropyLoss(ignore_index=-1, weight=self.alpha.to(re_prediction_scores))
            loss_fct_ner = CrossEntropyLoss(ignore_index=-1)
            re_loss = loss_fct_re(re_prediction_scores.view(-1, self.num_labels), labels.view(-1))  #
            ner_loss = loss_fct_ner(ner_prediction_scores.view(-1, self.num_ner_labels),ner_labels.view(-1))  # 两个参数，第一个应该是一个二维向量，指的是每个示例的预测得分
            # 交叉熵损失函数其实就是反映预测值和实际值之间的距离                                                        #第二个是实际的种类标签，一维标签即可
            loss = re_loss + ner_loss
            outputs = (loss, re_loss, ner_loss) + outputs

        return outputs# (masked_lm_loss), prediction_scores, (hidden_states), (attentions)
    # 0代表loss，1代表re loss（subject和object的预测值相加作为预测得分计算），2代表ner loss（预测的是object的实体类型），3代表re预测得分，4代表ner预测得分

class BertForACEBothOneDropoutSub_1_span(BertPreTrainedModel):#re修改
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
        ...
        #改进之处，使用双线性变换层，综合考虑subject和objec之间的关系
        self.reduce_dim = nn.Linear(config.hidden_size * 2, config.hidden_size)
        self.reduce_dimdou = nn.Linear(config.hidden_size * 4, config.hidden_size)
        self.blinear = nn.Bilinear(config.hidden_size, config.hidden_size, self.num_labels)
    ...
    def forward(
        ...
        feature_vector = torch.cat([e1_hidden_states, e2_hidden_states, m2_span_start_states, m2_span_end_states], dim=2)#在ner阶段就考虑相对关系
        ...
        #改进方法,先降维是为了减少运算量，GELU与随机正则化有关，使用该函数可以让得到的向量变得更加规范
        m1 = F.gelu(self.reduce_dim(m1_states))
        m2 = F.gelu(self.reduce_dimdou(feature_vector))
        dimenson = m1_states.shape[0]
        for i in range(0,dimenson):
            m2_1 = m2[i]
            m1_1 = m1[i]
...
class BertForACEBothOneDropoutSub(BertPreTrainedModel):# re
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
    ...
    def forward(
        ...
        m1_start_states = hidden_states[torch.arange(bsz), sub_positions[:, 0]]
        m1_end_states = hidden_states[torch.arange(bsz), sub_positions[:, 1]]#
        m1_states = torch.cat([m1_start_states, m1_end_states], dim=-1)#得到的是subject的开始结束信息
...
"""class BertForACEBothOneDropoutSub(BertPreTrainedModel):# re的prompt修改
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
        ...
        self.re_classifier_m1 = nn.Linear(config.hidden_size * 2, self.num_labels)
        self.re_classifier_m2 = nn.Linear(config.hidden_size * 2, self.num_labels)
        self.re_classifier_mask = nn.Linear(config.hidden_size , self.num_labels)
    ...
    def forward(
        ...
        # 获取mask位置的输出，看看预测的是什么
        dimenson = feature_vector.shape[0]
        for i in range(0, dimenson):
            masked_hidden_states.append(hidden_states[i, mask_id[i, 0]])
        masked_hidden_states = torch.stack(masked_hidden_states)  # 转化为tensor
        masked_hidden_states = torch.reshape(masked_hidden_states, (dimenson, 1, 768))

        mask_prediction_scores = self.re_classifier_mask(masked_hidden_states)
        ...
        #修改输出
        outputs = (mask_prediction_scores, ner_prediction_scores) + outputs[2:]
        ...
        if labels is not None:  # 计算损失
            ...
            #prompt
            loss_fct_mask = CrossEntropyLoss(ignore_index=-1, weight=self.alpha.to(mask_prediction_scores))
            mask_loss = loss_fct_mask(mask_prediction_scores.view(-1, self.num_labels), labels.view(-1))

            loss = loss + mask_loss

            outputs = (loss, mask_loss, ner_loss) + outputs

        return outputs# (masked_lm_loss), prediction_scores, (hidden_states), (attentions)
    # 0代表loss，1代表re loss（subject和object的预测值相加作为预测得分计算），2代表ner loss（预测的是object的实体类型），3代表re预测得分，4代表ner预测得分
"""

class BertForACEBothOneDropoutLeviPair(BertPreTrainedModel):#levitated marker方法
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
    ...
    def forward(
        ...
        m1_feature_vector = torch.cat([e1_hidden_states, e2_hidden_states], dim=2)#subject
        m2_feature_vector = torch.cat([e3_hidden_states, e4_hidden_states], dim=2)#object
...
class BertForACEBothOneDropoutLeviPair_blinear(BertPreTrainedModel):  # levitated marker方法
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
        ...
        # 使用Bilinear可以更好地考虑两个实体之间地关系(改进方法)
        self.bire_classifier = nn.Bilinear(config.hidden_size * 2, config.hidden_size * 2, self.num_labels)
    ...
    def forward(
        ...
        # 改进方法
        re_prediction_scores_biliner = self.bire_classifier(m1_feature_vector,m2_feature_vector)
        re_prediction_scores = re_prediction_scores + re_prediction_scores_biliner
...
class BertForACEBothOneDropoutLeviPair_blinear_1(BertPreTrainedModel):  # levitated marker方法
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
        ...
        # 进一步改进
        self.reduce_dim = nn.Linear(config.hidden_size * 2, config.hidden_size)
        self.bire_classifier = nn.Bilinear(config.hidden_size, config.hidden_size, self.num_labels)
    ...
    def forward(
        ...
        # 进一步改进
        m1_feature_vector = F.gelu(self.reduce_dim(m1_feature_vector))
        m2_feature_vector = F.gelu(self.reduce_dim(m2_feature_vector))
        re_prediction_scores_biliner = self.bire_classifier(m1_feature_vector, m2_feature_vector)
        re_prediction_scores = re_prediction_scores + re_prediction_scores_biliner
...
"""class BertForSpanMarkerNER(BertPreTrainedModel):  # ner_prompt阶段使用的模型
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
        ...
        #prompt的修改
        self.ner_mask_classifier = nn.Linear(config.hidden_size, self.num_labels)
    ...
    def forward(
        ...
        hidden_states = outputs[0]  #sequence_output = encoder_outputs[0]  ，encoder_outputs[0]就是bert的最后一层输出的hideenstate
        # pooled_output = self.pooler(sequence_output)，这个就是调用BertPooler方法，获得cls的输出
        # outputs = (sequence_output, pooled_output,) + encoder_outputs[1:]
        if self.onedropout:
            hidden_states = self.dropout(hidden_states)  # dropout在此处的作用就是随机使得一些变量为0，即丢弃这些部分的变量，但是注意为了使得输出得值正确，还需要等比例放大

        seq_len = self.max_seq_length
        bsz, tot_seq_len = input_ids.shape
        ent_len = (tot_seq_len - seq_len) // 2  # 一个样本sentence中最多有多少个实体

        e1_hidden_states = hidden_states[:, seq_len:seq_len + ent_len]  # 实体的开始边界信息
        e2_hidden_states = hidden_states[:, seq_len + ent_len:]  # 实体得结束边界信息，实体的边界信息是（256*768），每个边界是1*768
        # hiddenstate是8*1024*768
        # mention_pos存储的就是二元组（x，y），代表的是一个实体的subword边界
        m1_start_states = hidden_states[torch.arange(bsz).unsqueeze(-1), mention_pos[:, :,0]]  # m1_start_states 8*256*768，每个sentence即样本的mention_pos[:, :, 0]有256个，故，此处是把这256个实体开始标记都提取出来
        m1_end_states = hidden_states[torch.arange(bsz).unsqueeze(-1), mention_pos[:, :, 1]]
        # unsqueeze(-1)将8个元素组成的一维数组变成8*1，即[[],[],[],[],[],[]]这种形式
        feature_vector = torch.cat([e1_hidden_states, e2_hidden_states, m1_start_states, m1_end_states],dim=2)  # 8*256*3072（4*768）
        ...
        if not self.onedropout:  # 此处的feature_vector256中的每一行都代表一个实体（包括实体的位置信息以及实体本身内容的信息，即T-concat和悬浮标记的结合）
            feature_vector = self.dropout(feature_vector)
            masked_hidden_states = self.dropout(masked_hidden_states)

        ner_prediction_scores = self.ner_classifier(feature_vector)  # 一个线形层将实体映射到每个类别上的得分
        # 8*256*7，7代表label数

        #prompt的修改
        masked_prediction_scores = self.ner_mask_classifier(masked_hidden_states)


        outputs = (ner_prediction_scores,) + outputs[2:]  # Add hidden states and attention if they are here
        #prompt的修改
        outputs = (masked_prediction_scores,) + outputs[2:]  # Add hidden states and attention if they are here

        if labels is not None:#这个labels就是所预测的实体的种类的标签，这个是训练阶段就使用的
            loss_fct_ner = CrossEntropyLoss(ignore_index=-1, weight=self.alpha.to(ner_prediction_scores))
            ner_loss = loss_fct_ner(ner_prediction_scores.view(-1, self.num_labels), labels.view(-1))  #得到一个损失分数

            #prompt的修改
            loss_fct_ner_mask = CrossEntropyLoss(ignore_index=-1, weight=self.alpha.to(masked_prediction_scores))
            ner_mask_loss = loss_fct_ner_mask(masked_prediction_scores.view(-1, self.num_labels), labels.view(-1))
            ner_loss = ner_loss + ner_mask_loss


            outputs = (ner_loss,) + outputs  # 交叉熵损失函数很简单，先取softmax，再取对数，在与真实值相乘相加，即得到损失分数
            # view（-1）会使其变成一个一维张量，view（-1，a）指不知道行数，但知道列数，因此按这个展开
        return outputs  # 0为损失，1为分类后的得分，后面则为bert的对应位置的输出
"""



class BertForSpanMarkerNER(BertPreTrainedModel):  # ner阶段使用的模型
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
...
class BertForSpanMarkerNER_onlymarker(BertPreTrainedModel):#
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
...
class BertForSpanMarkerNER_onlyentity(BertPreTrainedModel):#
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
...
class BertForSpanMarkerBiNER(BertPreTrainedModel):#使用双线性层的ner阶段
    def __init__(self, config):
        super().__init__(config)
        self.max_seq_length = config.max_seq_length
        self.num_labels = config.num_labels
        ...
        self.onedropout = config.onedropout
        self.reduce_dim = nn.Linear(config.hidden_size*2, config.hidden_size)
        self.blinear = nn.Bilinear(config.hidden_size, config.hidden_size, self.num_labels)#使用双线性层来可以更好的考虑两个张量之间的关系
        self.init_weights()
