...
from transformers import (WEIGHTS_NAME, BertConfig,
                         ...
                         #BertForACEBothOneDropoutSub_prompt,
                          AlbertForACEBothSub,
                          AlbertConfig,
                          AlbertTokenizer,
...
MODEL_CLASSES = {
    ...
    #'bertsub_prompt': (BertConfig, BertForACEBothOneDropoutSub_prompt, BertTokenizer),
...
data_dir = {'data_path_loss': 'train_re_prompt_2_loss.txt',
            'data_name_loss': 'train_re_prompt_2_loss', 'data_path_f1': 'train_re_prompt_2_f1.txt',
            'data_name_f1': 'train_re_prompt_2_f1'}
...
class ACEDataset(Dataset):
    ...
    def initialize(self):
        ...
        max_num_subwords = self.max_seq_length - 4  # for two marker  252  256-固定标记-cls和seq
        ...
        for l_idx, line in enumerate(f):  # 每次读一行sentences，包括很多个句子
            ...
            for n in range(len(subword_sentence_boundaries) - 1):  # n个句子sentence
                ...
                target_tokens = subwords[doc_offset: doc_sent_end + right_context_length]  # 当前句子的上下文信息

                #最大的长度，由于加入了prompt，因此此处也需要改变，-4便为-14
                target_tokens = [tokenizer.cls_token] + target_tokens[: self.max_seq_length - 20] +[tokenizer.sep_token]
                ...
                for x in sentence_relations:
                    ...
                    self.golden_labels.add(((l_idx, n), (x[0], x[1]), (x[2], x[3]),
                                            x[4]))  # golden_labels_withner和golden_labels的区别在于加入了实体类型
                ...
                for sub in entities:  # sub代表一个实体信息（token信息）
                    ...
                    if sub[0] < 10000:  # 如果实体的开始边界<10000
                        ...
                        #prompt修改      第一个UNK代表object的类别，后面两个为开始和结束边界

                        list_sub = sub[2].split(",")
                        if sub_e == sub_s :
                            if self.args.prompt_index == 1:
                                sub_tokens = sub_tokens[:-1] + ["The"] + list_sub + target_tokens[sub_e:sub_e + 1] + ["[MASK]", "the", "[UNK]", "[UNK]", "[UNK]", "."] + sub_tokens[-1:]
                            if self.args.prompt_index == 2:
                                sub_tokens = sub_tokens[:-1] + ["The","[MASK]","is","described","by"] + list_sub + target_tokens[sub_e:sub_e + 1]+["and","[UNK]","[UNK]", "[UNK]","."] + sub_tokens[-1:]
                            if self.args.prompt_index == 3:
                                sub_tokens = sub_tokens[:-1] + ["The", "[MASK]", "is", "defined","by"] + list_sub + target_tokens[sub_e:sub_e + 1] + ["and", "[UNK]", "[UNK]", "[UNK]", "."] + sub_tokens[-1:]
                            if self.args.prompt_index == 4:
                                sub_tokens = sub_tokens[:-1] + ["A", "[MASK]", "known", "as"] + list_sub + target_tokens[sub_e:sub_e + 1] + ["and", "[UNK]", "[UNK]", "[UNK]", "."] + sub_tokens[-1:]
                        else:
                            if self.args.prompt_index == 1:
                                sub_tokens = sub_tokens[:-1] + ["The"] + list_sub +target_tokens[sub_s:sub_s + 1]+ target_tokens[sub_e:sub_e + 1] + [
                                    "[MASK]", "the", "[UNK]", "[UNK]", "[UNK]", "."] + sub_tokens[-1:]
                            if self.args.prompt_index == 2:
                                sub_tokens = sub_tokens[:-1] + ["The", "[MASK]", "is", "described",
                                                                "by"] + list_sub + target_tokens[sub_s:sub_s + 1]+target_tokens[sub_e:sub_e + 1] + [
                                                 "and", "[UNK]", "[UNK]", "[UNK]", "."] + sub_tokens[-1:]
                            if self.args.prompt_index == 3:
                                sub_tokens = sub_tokens[:-1] + ["The", "[MASK]", "is", "defined",
                                                                "by"] + list_sub + target_tokens[sub_s:sub_s + 1]+target_tokens[sub_e:sub_e + 1] + [
                                                 "and", "[UNK]", "[UNK]", "[UNK]", "."] + sub_tokens[-1:]
                            if self.args.prompt_index == 4:
                                sub_tokens = sub_tokens[:-1] + ["A", "[MASK]", "known",
                                                                "as"] + list_sub + target_tokens[sub_s:sub_s + 1]+target_tokens[sub_e:sub_e + 1] + [
                                                 "and", "[UNK]", "[UNK]", "[UNK]", "."] + sub_tokens[-1:]
                        ...
                        if self.args.prompt_index == 1:
                            ...
                            sub_tokens = target_tokens[:-1] + ["The"] + ['NIL'] + [
                                "[MASK]", "the", "[UNK]", "[UNK]", "[UNK]", "."] + target_tokens[-1:]

                        if self.args.prompt_index == 2:

                            sub_tokens = target_tokens[:-1] + ["The", "[MASK]", "is", "described",
                                                            "by","NIL","and", "[UNK]", "[UNK]", "[UNK]", "."] + target_tokens[-1:]

                        if self.args.prompt_index == 3:

                            sub_tokens = target_tokens[:-1] + ["The", "[MASK]", "is", "defined",
                                                            "by", "NIL","and",
                                                            "[UNK]","[UNK]","[UNK]","."] + target_tokens[-1:]

                        if self.args.prompt_index == 4:


                            sub_tokens = target_tokens[:-1] + ["A", "[MASK]", "known",
                                                            "as","NIL","and", "[UNK]", "[UNK]", "[UNK]", "."] + target_tokens[-1:]

                        sub_tokens = sub_tokens + ['[unused0]', '[unused1]']
                    ...
                    for start, end, obj_label in sentence_ners:  # 在这一步开始对object实体进行操作，一个subject遍历所有的object
                        ...
                        cur_ins.append(((left, right, ner_label_map[obj_label]), label,obj))  # 这个label指得是关系，obj是实体的位置信息，前者是object与subject的相对位置关系
                    ...
                    for i in range(0, len(cur_ins), dL):  # 这个地方就是代表了一个样本的关系数。最大为16
                        ...
                        if sub[0] < 10000:
                            # 加入object的实体信息
                            if self.args.prompt_index == 1:
                                sub_tokens[len(sub_tokens) - 5] = self.ner_label_list[examples[0][0][2]]
                            if self.args.prompt_index == 2:
                                sub_tokens[len(sub_tokens) - 5] = self.ner_label_list[examples[0][0][2]]
                            if self.args.prompt_index == 3:
                                sub_tokens[len(sub_tokens) - 5] = self.ner_label_list[examples[0][0][2]]
                            ...
                            if self.args.prompt_index == 1:
                                sub_tokens[len(sub_tokens) - 7] = self.ner_label_list[examples[0][0][2]]
                            if self.args.prompt_index == 2:
                                sub_tokens[len(sub_tokens) - 7] = self.ner_label_list[examples[0][0][2]]
                            if self.args.prompt_index == 3:
                                sub_tokens[len(sub_tokens) - 7] = self.ner_label_list[examples[0][0][2]]
    ...
    def __getitem__(self, idx):
        ...
        for x_idx, obj in enumerate(entry['examples']):
            ...
            if '[unused1]' != words[-1]:
                if self.args.prompt_index == 1:
                    position_ids[L - 4] = m2[0]  # l为input的长度
                    position_ids[L - 3] = m2[1]

                    input_ids[L - 4] = input_ids[m2[0]]  # 也把边界信息共享
                    input_ids[L - 3] = input_ids[m2[1]]
                if self.args.prompt_index == 2:
                    position_ids[L - 4] = m2[0]  # l为input的长度
                    position_ids[L - 3] = m2[1]
                ...
                if self.args.prompt_index == 1:
                    position_ids[L - 6] = m2[0]  # l为input的长度
                    position_ids[L - 5] = m2[1]

                    input_ids[L - 6] = input_ids[m2[0]]  # 也把边界信息共享
                    input_ids[L - 5] = input_ids[m2[1]]
                if self.args.prompt_index == 2:
                    position_ids[L - 6] = m2[0]  # l为input的长度
                    position_ids[L - 5] = m2[1]
        ...
        with torch.no_grad():
            ...
            for i in range(len(indexs)):  # 多少个样本
                ...
                for j in range(len(m2s)):  # 每个object
                    ...
                    logits[i, j].tolist(), ner_label)  # scores存储的就是每个subject和object对的关系
        ...
        for example_index, pair_dict in sorted(scores.items(), key=lambda x: x[0]):  # 从所有的
            ...
            for k1, (v1, v2_ner_label) in pair_dict.items():  # 开始对一对实体进行处理
                ...
                if v2s is not None:
                    ...
                    # 调整顺序的原因在于0-7是subject，8-12是object，阿紫颠倒顺序之后，爷爷奥将其对换
            ...
            for item in no_overlap:
                ...
                if pred_label in sym_labels:
                    ...
                    if (example_index, (m1[0], m1[1], pos2ner[m1]), (m2[0], m2[1], pos2ner[m2]),
                        pred_label) in golden_labels_withner:  # 判断是否也预测准了ner
                        cor_with_ner += 1
...
def main():
    ...
    parser.add_argument('--prompt_index', type=int, default=1,
                        help="random index for ACEDataset")
    ...
    config_class, model_class, tokenizer_class = MODEL_CLASSES[
        args.model_type]  # model_class = BertForACEBothOneDropoutSub
