...
MODEL_CLASSES = {
    ...
    #'bertprompt': (BertConfig, BertForSpanNER, BertTokenizer),
...
#data_dir={'data_path_loss':'train_ner_onlymarker_loss.txt','data_name_los s':'train_ner_onlymarker_loss','data_path_f1':'train_ner_onlymarker_f1.txt','data_name_f1':'train_ner_onlymarker_f1'}
...
class ACEDatasetNER(Dataset):
    ...
    def initialize(self):
        ...
        for l_idx, line in enumerate(f):
            ...
            for i in range(len(sentences)):
                ...
                sentence_boundaries.append(L)#boundaries是一个[0,22,60]类型的分隔符号,每个符号代表一个句子的结束处
           ...
           #拆分成subword
            ...
            for n in range(len(subword_sentence_boundaries) - 1):
                ...
                target_tokens = [tokenizer.cls_token] + target_tokens + [tokenizer.sep_token]#这才是输入到bert中的一个句子的标准形式
                ...
                for entity_start in range(left_context_length, left_context_length + sentence_length):
                    ...
                    for entity_end in range(entity_start + 1, left_context_length + sentence_length + 1):#其实就是遍历这个句子的长度
                        ...
                        if subword2token[doc_entity_end - 1] - subword2token[doc_entity_start] + 1 > self.args.max_mention_ori_length:#一旦两个词之间的距离过大，则会直接返回
                            continue
                ...
                if not self.args.group_edge:
                    for i in range(0, len(entity_infos), dL):#从0-len，其中dl为步进值
                        #entity_infos存放的是输入的句子中的所有可能的实体，因此examples存放的信息也是如此
                        ...
                        item = {#这个字典记录了每个句子中的实体关系信息，以及句子信息
                            ...
                            'example_L': len(entity_infos)
                        } #此处的item是送往dataset中的句子，即送入bert的句子，example相当于一种验证，用于和bert输出的识别结果做比对
                        self.data.append(item)
                else:
    ...
    def __getitem__(self, idx):
        ...
        #全注意力矩阵用于使得句子之间相互可见  example的内容((1, 1), 5, (0, 0))
        for x_idx, x in enumerate(entry['examples']):
            m1 = x[0]
            label = x[1]
            mentions.append(x[2])#实体的word边界信息
        ...
        labels += [-1] * (num_pair - len(labels))#同理，补全至256个
        mention_pos += [(0, 0)] * (num_pair - len(mention_pos))#用（0，0）的位置信息来补全256个实体边界的信息
...
def train(args, model, tokenizer):
    ...
    train_sampler = RandomSampler(train_dataset) if args.local_rank == -1 else DistributedSampler(train_dataset)#对原来的数据集打乱操作
    train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=args.train_batch_size, num_workers=2*int(args.output_dir.find('test')==-1))#simper相当于定义了怎么从dataset里取samper，相当于一种策略，即随机取策略，
    #train_dataloader中会有batchsample参数，其实就是一个batchsize的sample所组成的集合
    if args.max_steps > 0:
        t_total = args.max_steps
        args.num_train_epochs = args.max_steps // (len(train_dataloader) // args.gradient_accumulation_steps) + 1
    else:
        t_total = len(train_dataloader) // args.gradient_accumulation_steps * args.num_train_epochs#//代表取整除，gradient_accumulation_steps是累计的梯度次数，每gradient_accumulation_steps次进行一次梯度更新
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in model.named_parameters() if not any(nd in n for nd in no_decay)], 'weight_decay': args.weight_decay},#weight_decay是权重衰减值，可以对参数正则化，用于防止过拟合，防止梯度爆炸
    ...
    if args.warmup_steps==-1:#刚开始训练时,模型的权重(weights)是随机初始化的，此时若选择一个较大的学习率,可能带来模型的不稳定(振荡)，lr先慢慢增加，超过warmup_steps时，就采用初始设置的学习率
        scheduler = get_linear_schedule_with_warmup(#scheduler是用来调节学习率的
            optimizer, num_warmup_steps=int(0.1*t_total), num_training_steps=t_total#num_warmup_steps是预热步骤数，num_training_steps是训练步数
        )
    else:
    ...
    if args.fp16:#使用16bit的浮点数，可以减小缓存，但是会产生误差和溢出（pytorch默认张量精度为32位）
        ...
        model, optimizer = amp.initialize(model, optimizer, opt_level=args.fp16_opt_level)#使用预定的opt_level对模型初始化，使用01等级
    # ori_model = model
    ...
    model.zero_grad()
    train_iterator = trange(int(args.num_train_epochs), desc="Epoch", disable=args.local_rank not in [-1, 0])#trange(i) 是 tqdm(range(i)) 的简单写法
        ...
        with torch.no_grad():
            ...
            for i in range(len(indexs)):#共有多少个实体
                ...
                for j in range(len(m2s)):#样本中的第几个实体span
                    ...
                    if ner_label!='NIL':
                        scores[(index[0], index[1])][(obj[0], obj[1])] = (float(ner_values[i,j]), ner_label)#scores[i，j)][x，y]，i，j表示句子的索引，xy表示边界信息
