...
data_dir = {'data_path_loss': 'train_re_span_loss.txt', 'data_name_loss': 'train_re_span_loss',
            'data_path_f1': 'train_re_span_f1.txt', 'data_name_f1': 'train_re_span_f1'}
    ...
    def initialize(self):
        ...
        for l_idx, line in enumerate(f):  # 每次读一行sentences，包括很多个句子
            ...
            if 'predicted_ner' in data:  # e2e predict
                ...
                ners = data[
                    'ner']  # 这些位置信息都是一个sentences数据中的位置，但是由于列表的性质，每个对应的列表元素都是一个单独的sentence，包括对应的ner，re[0]对应的位置也都是这样
            ...
            for n in range(len(subword_sentence_boundaries) - 1):  # n个句子sentence
                ...
                self.global_predicted_ners[(l_idx, n)] = list(
                    sentence_ners)  # global_predicted_ners[(l_idx, n)]里面存储了某一行某个句子所有的实体
                ...
                for x in sentence_relations:  # 某一个sentence中的关系
                    ...
                    if w not in pos2label:
                        if x[4] in self.sym_labels[
                                   1:]:  # 如果是对称的关系，则加入pos2label中，因为在上述中，只是将对称关系的关系信息加入到了golden_labels中，但是并未加入到pos2label中
                ...
                for sub in entities:  # sub代表一个实体信息（token信息）
                    ...
                    if sub[0] < 10000:  # 如果实体的开始边界<10000
                        ...
                        if self.use_typemarker:  # 此处用于定义是否将实体信息也加入到文本中
                            ...
                            r_m = '[unused%d]' % (2 + sub_label + len(
                                self.ner_label_list))  # 根据之后的代码，可以判断，此处的2是说unused0和1已经被占用，剩下的，每ner_label_list长度为一个单位，顺序对应着subject和object的ner标签
                        else:  # 注：在vocab词汇表中，unused0的索引为1，以此类推
                            l_m = '[unused0]'
                            r_m = '[unused1]'

                        sub_tokens = target_tokens[:sub_s] + [l_m] + target_tokens[sub_s:sub_e + 1] + [
                            r_m] + target_tokens[sub_e + 1:]  # 经过这一步，其实开始何结束位置指向的已经是这个标签的位置
                    ...
                    for start, end, obj_label in sentence_ners:  # 在这一步开始对object实体进行操作，一个subject遍历所有的object
                        ...
                        cur_ins.append(((left, right, ner_label_map[obj_label]), label,
                                        obj))  # 这个label指得是关系，obj是实体的位置信息，前者可能是object与subject的相对我位置关系
...
def train(args, model, tokenizer):
    ...
    for _ in train_iterator:  # 开始
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            re_loss = outputs[
                1]  # 0代表loss，1代表re loss（subject和object的预测值相加作为预测得分计算），2代表ner loss（预测的是object的实体类型），3代表re预测得分，4代表ner预测得分
            ner_loss = outputs[2]
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    if not args.eval_unidirect:  # eval_unidrect is for ablation study
        ...
        for example_index, pair_dict in sorted(scores.items(), key=lambda x: x[0]):  # 从所有的
            ...
            for item in no_overlap:
                ...
                # 新增列表来存储直观的数据
                file_path = os.path.join('scierc/ner_result.json')
                ...
                output_preds_1.append((sentence[m1[0] - length:m1[1] + 1 - length],
                                       sentence[m2[0] - length:m2[1] + 1 - length], pred_label))
    ...
    if do_test:
        #output_w = open(os.path.join('/home/PL-Marker/scierc/', 're_result.json'), 'w')
        #json.dump(tot_output_results, output_w)

        # 新增保存具体实体的文件
        output_w_1 = open(os.path.join('/home/PL-Marker/scierc/', 're_result.json'), 'w')
